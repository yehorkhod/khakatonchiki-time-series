...
import numpy as np
import pandas as pd
from preprocess import Preprocess, Config  # Импорт класса Preprocess
...
class Models:
    ...
    def NeuralProphet(X_train, y_train, X_test, y_test, date_col_train, date_col_test):
        ...
        try:
            ...
            np_model.fit(np_train, freq='D')  # Добавлен freq параметр

            future = np_model.make_future_dataframe(np_train, periods=len(y_test))
            ...
            print(f"Error in NeuralProphet: {e}")
            return None'''
...
class AutoGluonModel:
    ...
    def train_and_evaluate(train_data, test_data, target_column, prediction_length):
        ...
        return {
            ...
            "mae": evaluation.get("MAE", float("nan")),
